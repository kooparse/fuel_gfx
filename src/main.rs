...
use gfx_hal::{
    ...
    pass::{
        ...
        Subpass, SubpassDependency, SubpassDesc, SubpassRef,
    },
    pool::CommandPoolCreateFlags,
    pso::{
        BlendState, ColorBlendDesc, ColorMask, EntryPoint,
        GraphicsPipelineDesc, GraphicsShaderSet, PipelineStage, Rasterizer,
        Rect, Viewport,
    },
    queue::Submission,
    Backbuffer, Device, FrameSync, Graphics, Instance, Primitive, Surface,
    SwapImageIndex, Swapchain, SwapchainConfig,
};

use winit::{
    Event, EventsLoop, KeyboardInput, VirtualKeyCode, WindowBuilder,
    WindowEvent,
};

fn main() {
    // Create our window event loop.
    ...
    let instance = backend::Instance::create("Fuel", 1);
    ...
    // surface supports to allocate queues from.
    let num_queues = 1;
    ...
    let vertex_shader_module = {
        let spirv = include_bytes!("../assets/shaders/default.vert.spv");
        device.create_shader_module(spirv).unwrap()
    };

    let fragment_shader_module = {
        let spirv = include_bytes!("../assets/shaders/default.frag.spv");
        device.create_shader_module(spirv).unwrap()
    };
